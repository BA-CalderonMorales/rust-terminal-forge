...
// Secure chart implementation without dangerouslySetInnerHTML
...
>(({ id, className, children, config, ...props }, ref) => {
  ...
  // Generate CSS variables securely using React's style prop
  const cssVariables = React.useMemo(() => {
    ...
    Object.entries(config).forEach(([key, itemConfig]) => {
      if (itemConfig.theme || itemConfig.color) {
        ...
        if (lightColor) {
          variables[`--color-${sanitizedKey}`] = lightColor
        }
        if (darkColor && itemConfig.theme) {
          variables[`--color-${sanitizedKey}-dark`] = darkColor
        }
      }
    })
...
// Keep all other chart components the same but remove ChartStyle
