...
import { themeManager, themes as existingThemes } from '../theme';
...
export const professionalThemes: ProfessionalTheme[] = [
  {
    id: 'default',
    name: 'Default Dark',
...
export function ProfessionalThemeProvider({ children }: { children: React.ReactNode }) {
  const [currentTheme, setCurrentTheme] = useState<ProfessionalTheme>(professionalThemes[0]);

  useEffect(() => {
    // Try to get theme from existing theme manager first
    const existingTheme = themeManager.getCurrentTheme();
    
    // Map existing theme to professional theme if possible
    const mappedTheme = professionalThemes.find(theme => 
      theme.id === existingTheme.name.toLowerCase().replace(/\s+/g, '') ||
      theme.name.toLowerCase() === existingTheme.name.toLowerCase()
    );
    
    if (mappedTheme) {
      setCurrentTheme(mappedTheme);
    } else {
      // Check localStorage for professional theme
      const savedTheme = localStorage.getItem('professional-terminal-theme');
      if (savedTheme) {
        const theme = professionalThemes.find(t => t.id === savedTheme);
        if (theme) {
  ...
  useEffect(() => {
    // Apply theme to CSS variables for professional components
    ...
    // Map to existing terminal theme variables for compatibility
    root.style.setProperty('--terminal-bg', colors.background);
    root.style.setProperty('--terminal-bg-secondary', colors.surface);
    root.style.setProperty('--terminal-border-default', colors.border);
    root.style.setProperty('--terminal-fg', colors.text);
    root.style.setProperty('--terminal-fg-secondary', colors.textSecondary);
    root.style.setProperty('--terminal-fg-muted', colors.textMuted);
    root.style.setProperty('--terminal-accent-primary', colors.primary);
    root.style.setProperty('--terminal-accent-success', colors.success);
    root.style.setProperty('--terminal-accent-warning', colors.warning);
    root.style.setProperty('--terminal-accent-error', colors.error);
    root.style.setProperty('--terminal-neon-green', colors.primary);
    
    // Maintain compatibility with existing theme manager
    try {
      ...
      if (existingThemeKey && themeManager.getCurrentTheme().name !== currentTheme.name) {
        themeManager.setTheme(existingThemeKey);
      }
    } catch (error) {
      console.warn('Could not sync with existing theme manager:', error);
    }
  }, [currentTheme]);

  const setTheme = (themeId: string) => {
    const theme = professionalThemes.find(t => t.id === themeId);
    if (theme) {
      setCurrentTheme(theme);
      localStorage.setItem('professional-terminal-theme', themeId);
    }
  };

  return (
    <ProfessionalThemeContext.Provider value={{ currentTheme, setTheme, themes: professionalThemes }}>
      {children}
